...
const CustomImagePicker = ({
  textAAficher, setSelectedImage, error,  name, driverDocs, onChange
}) => {
  ...
  const captureImage = async type => {
    ...
    if (isCameraPermitted && isStoragePermitted) {
      launchCamera(options, response => {
        ...
        onChange({ localUri: response.assets[0].uri })
        driverDocs.current = [
          ...driverDocs.current,
          {
  ...
  const cancelPhoto = () => {
    ...
    onChange(null)
    if(driverDocs.current && driverDocs.current.length){
      let fitredDriverDocs = driverDocs.current.filter(x => x.name !== name)
      fitredDriverDocs = [
        ...fitredDriverDocs
      ];
    }
        ...
        {filePath?.assets[0].uri ? (
          ...
          <View style={{...styles.imagePosition}}>
            ...
            <Image
              ...
              style={{...styles.photoResult, borderWidth:2 , borderColor: 'green'}}
              resizeMode={'cover'}
            />
          ...
          <View style={{...styles.capturePhoto, borderWidth:2 , borderColor: error ? 'red' : 'green'}}>
